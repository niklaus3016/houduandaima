...
async function checkCuidingTeamNewUsers() {
  try {
    // 连接MongoDB
    ...
    for (const employee of employees) {
      ...
      if (loginRecords.length > 0) {
        ...
        onlineEmployees.push({
          ...
          lastLogin: loginRecords[loginRecords.length - 1].loginDate
        });
      } else {
        offlineCount++;
    ...
    if (onlineEmployees.length > 0) {
      ...
      onlineEmployees.forEach((emp, index) => {
        console.log(`${index + 1}. ${emp.employeeId} - ${emp.name} (登录${emp.loginCount}次)`);
        console.log(`   最后登录: ${new Date(emp.lastLogin.getTime() + 8 * 60 * 60 * 1000).toLocaleString()}`);
      });
    }
