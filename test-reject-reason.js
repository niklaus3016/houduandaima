...
async function testRejectReasonField() {
  try {
    // 超管登录
    console.log('========================================');
    ...
    if (pendingResponse.data.data.records.length > 0) {
      ...
      if (firstRecord.rejectReason !== undefined) {
        console.log('✅ 待处理接口返回了rejectReason字段');
      } else {
        console.log('❌ 待处理接口未返回rejectReason字段');
      }
    }
    console.log('');
